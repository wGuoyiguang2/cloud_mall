...
import com.cibnvideo.tcmalladmin.model.bean.PriceProduct;
...
public interface ProductPricePolicyApi {
    @RequestMapping(value = Router.V1_OMS_PRICE_POLICY_PRODUCT_LIST, method = RequestMethod.GET)
    ResultData<DataList<List<PriceProduct>>> list(@RequestParam("params") Map<String, Object> params);

    @RequestMapping(value = Router.V1_OMS_PRICE_POLICY_PRODUCT_GET, method = RequestMethod.GET)
    ResultData<PriceProduct> get(@RequestParam("id") Integer id);
    ...
    ResultData<Integer> add(@RequestBody PriceProduct priceProduct);

    @RequestMapping(value = Router.V1_OMS_PRICE_POLICY_PRODUCT_UPDATE, method = RequestMethod.POST)
    ResultData<Integer> update(@RequestBody PriceProduct priceProduct);
