package com.hqtc.ims.common.config.datasource;
...
@Aspect
@Component
public class ImsDbAop {

    private Logger logger = LoggerFactory.getLogger("ImsDbAop");

    @Pointcut("execution(public * com.hqtc.ims.address.model.mapper.*.*(..)) || " +
            "execution(public * com.hqtc.ims.favorite.model.mapper.*.*(..)) || " +
            "execution(public * com.hqtc.ims.invoice.model.mapper.*.*(..)) || " +
            "execution(public * com.hqtc.ims.searchhistory.model.mapper.*.*(..))")
    ...
    public void afterMasterCall(){
        logger.info("local-thread-clear-BMS-DB");
